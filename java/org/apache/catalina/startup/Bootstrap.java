...
public final class Bootstrap {
    ...
    private void initClassLoaders() {
        try {
            ...
            // 定义“通用”类加载器内容的逗号分隔路径列表。
            // Prefixes should be used to define what is the repository type.
            // 应该使用前缀来定义存储库类型。
            // Path may be relative to the CATALINA_HOME or CATALINA_BASE path or absolute.
            // 路径可能相对于 CATALINA_HOME 或 CATALINA_BASE 路径或绝对路径。
            // If left as blank,the JVM system loader will be used as Catalina's "common" loader.
            // 如果留空，JVM 系统加载器将用作 Catalina 的“通用”加载器。
            // Examples:
            //     "foo": Add this folder as a class repository
            //     "foo/*.jar": Add all the JARs of the specified folder as class  repositories
            //     "foo/bar.jar": Add bar.jar as a class repository
            // Note: Values are enclosed in double quotes ("...") in case either the
            //     D:/apache-tomcat-9.0.41 path or the ${catalina.home} path contains a comma.
            //     Because double quotes are used for quoting, the double quote character  may not appear in a path.
    ...
    private ClassLoader createClassLoader(String name, ClassLoader parent)
            throws Exception {
        // 从catalina.properties这个配置文件中读取通用类加载器的名
        String value = CatalinaProperties.getProperty(name + ".loader");
        if ((value == null) || ("".equals(value))) {
            return parent;
        }
        // 将该参数的真实属性值从配置文件值的格式中解析出来，例如配置文件中可能是${}的形式
        value = replace(value);

        List<Repository> repositories = new ArrayList<>();
        // 获取配置文件中配置的类加载器要加载的代码仓的路径，例如：
        // "D:/apache-tomcat-9.0.41/lib","D:/apache-tomcat-9.0.41/lib/*.jar","${catalina.home}/lib","${catalina.home}/lib/*.jar"
        String[] repositoryPaths = getPaths(value);

        for (String repository : repositoryPaths) {
            // 首先尝试用Url去加载配置的路径，如果URL加载不了，尝试用*.jar，jar，dir等方式加载到repositories中
    ...
    public void init() throws Exception {
        ...
        // 获取Catalina类实例，Catalina 的启动/关闭 shell 程序
        Class<?> startupClass = catalinaLoader.loadClass("org.apache.catalina.startup.Catalina");
        Object startupInstance = startupClass.getConstructor().newInstance();

        // Set the shared extensions class loader
        // 设置共享扩展类加载器；调用Catalina的setPartClassLoader方法，设置父加载器为共享类加载器；
        // 默认为CommonClassLoader
        if (log.isDebugEnabled()) {
            log.debug("Setting startup class properties");
        }
        String methodName = "setParentClassLoader";
        // 参数类型数组，第一个类型是java.Classloader
        Class<?> paramTypes[] = new Class[1];
        paramTypes[0] = Class.forName("java.lang.ClassLoader");
        // 参数值数组，和类型数组对应，第一个类型对应的值是，sharedLoader
        ...
        // 将初始化时的Catalina类实例设置为catalina守护线程
